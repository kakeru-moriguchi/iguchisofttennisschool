...
export default function PriceTable() {
  return (
    <div className="hidden overflow-hidden rounded-2xl border border-navy-100 shadow-sm md:block">
      <table className="w-full border-collapse text-left">
        ...
        <thead>
          <tr className="bg-navy-800 text-white">
            ...
            {priceColumns.map((col) => (
              ...
              >
                {col.label}
                <span className="mt-0.5 block text-[11px] font-normal text-navy-200">
                  {col.note}
                </span>
              </th>
            ))}
        ...
        <tbody className="divide-y divide-navy-100 bg-white">
          {priceRows.map((row) => (
            <tr key={row.key} className="transition-colors hover:bg-navy-50">
              <th scope="row" className="px-6 py-5">
                <Link
                  href={`/classes#${row.classSlug}`}
                  className="text-base font-bold text-navy-800 transition-colors hover:text-sky-brand-dark"
                >
                  {row.className}
                </Link>
              </th>
